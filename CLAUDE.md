...
### Tabelle & Spielplan (standings / fixtures)

Fed entirely by `data/uzwil4.json` — **manually maintained**, no live third-party widget (an earlier version embedded a live football.ch SFV widget; that was replaced). The JSON has a `standings` array and a `fixtures` array. Inline JS at the bottom of `index.html` (`tsRenderStandings`, `tsFindNextMatch`, `tsRenderNextMatchHome`, around line 1343) `fetch()`s the file client-side, renders the standings table into `#standingsWrap`/`#standingsBody`, and finds/renders the next Uzwil fixture into the homepage `.match-card--next`. Team rows matching `/\buzwil\b/i` are highlighted and relabeled "NK Croatia Uzwil".

**After every matchday**: update `data/uzwil4.json` by hand (bump `standings`, add/fill in the played fixture's score in `fixtures`, bump `updated`). No build step or deploy script — just edit the JSON.

### PWA / offline

`manifest.json` (icons: `apple-touch-icon.png`, `favicon.ico`) + `sw.js` (cache name `nk-croatia-v1`) registered from `index.html` (`navigator.serviceWorker.register('/sw.js')`, ~line 1693). Precaches `index.html`, `style.css`, `logo.png`, `favicon.ico`, `apple-touch-icon.png`, `hero-video.mp4`; falls back to cached `index.html` when offline. **Bump `CACHE` in `sw.js` when precached assets change**, or returning visitors keep stale cached files.
...
- don't upload CLAUDE.md file anywhere
- do not publish on the Plesk automatically, you will get instructions when to push the changes

## Assets

- `logo.png` — club crest (transparent background); use class `mol-logo--transparent` or `footer-logo-img--transparent` where the background varies (`logo.jpeg` is a leftover, unused — safe to ignore/remove)
