# Android fan app — build prompt

Paste everything below this line into a fresh Claude Code / Android Studio agent
session. It's self-contained and doesn't need this repo's `CLAUDE.md` or any
prior conversation — everything the agent needs is in here.

---

Build a native Android app in **Kotlin + Jetpack Compose** (Coroutines/Flow,
MVVM with `ViewModel`/`StateFlow`, **minSdk 26 / target latest stable SDK**)
that shows fans of the football club NK Croatia Uzwil what's happening on
their website — league table, fixtures, results, live match score, news, and
roster. This is a **brand-new, separate** Android Studio project
(`nkcu_fan_android`). There is no existing Android code to build on, and this
is **not** an admin tool — there is no login, no editing, no write requests
...
firing an implicit `Intent` to the existing mailto: links (opens the user's
mail app) instead of reimplementing them. If in doubt whether something is in
scope, it isn't — ask before adding it.

## Backend — read-only, already live, do not modify

Base URL for development: `https://test26.croatia-uzwil.ch/` (staging). Structure
the networking layer (Retrofit + `kotlinx.serialization` or Moshi — your call,
be consistent) so this base URL is a single config value — production base URL
will be swapped in later (likely `https://croatia-uzwil.ch/`).
...
`RegexOption.IGNORE_CASE`) — this matches "FC Uzwil 4" today but is deliberately
loose because the team name can change between seasons. When a row matches,
visually highlight it (brand red/blue) and **relabel the displayed name to
"NK Croatia Uzwil"** instead of showing "FC Uzwil 4" — the underlying JSON
keeps the SFV-registered name because that's what the league association uses.
...
- `date` is `DD.MM.YYYY`, `time` is `HH:mm` (24h) and can be an **empty
  string** for a not-yet-scheduled kickoff time — handle that (don't crash
  parsing an empty `LocalTime`).
...
tracked live; otherwise treat a 404 or an empty `matches` object as "no live
match."
...
- `formation` is a string like `"4-4-2"` or `"3-5-2"`; `lineup` keys are
  ...
  don't hardcode a position list, render whatever keys are present (iterate
  the map).
- `side` on a goal is `"home"` or `"away"`; `scorer` is `"Gegner"` (German for
  "opponent") for an opposition goal, or the actual player name for a
  home-side goal scored by Uzwil (side is `"home"` in that row, but home isn't
  always Uzwil — check which of `home`/`away` matched `\buzwil\b` to know
  which side is "us" for coloring).
- `live: true` means the match is currently in progress — show a live
  ...
  while this screen is on-screen (cancel the polling coroutine in
  `onStop`/when the composable leaves composition). `live: false` with scores
  present means the match ended; stop polling and treat it like a normal result.
- Build the live scoreboard, goal timeline, and lineup/formation view as
  **fully native Compose UI** — no `WebView` anywhere in this feature. This is
  the one part of the spec the club owner called out explicitly.

### Team crests

`GET {base}/logos/{Team Name}.gif` — team name with spaces kept as literal
spaces in the path, URL-encode when building the request. Not every team has a
logo file; treat a failed load as "no logo" and fall back to a generic
placeholder crest (a plain circle/monogram is fine) via Coil's `error()`
painter — mirror the website's `onerror` fallback behavior, don't show a
broken-image glyph.
...
`<strong>`/`<b>`, `<em>`/`<i>` (sanitized server-side on write) — render it in
the detail screen with `AnnotatedString`/`HtmlCompat.fromHtml(...)` (no
`WebView` needed for a tag set this small); `excerpt` is a pre-truncated
plain-text summary, safe to show as plain `Text`. This feed is read-only from
this app's side — publishing happens from a separate admin app, not from here.
...
For v1:
- Ship a **small bundled JSON snapshot** (`res/raw/roster_seed.json`) with a
  handful of realistic placeholder entries in the shape you'd expect a future
  `data/roster.json` to return (name/photo/role per entry, role text in
  German and Croatian).
- Architect the data layer (repository pattern) so swapping the bundled seed
  for a real `GET {base}/data/roster.json` later is a one-line change (same
  repository pattern as the news/table/fixtures calls) — don't hardcode "load
  from raw resource" deep inside the Composable layer.
...
5. **News** — list + detail screen, backed by the live `data/news.json` feed.
...
7. **Info / Settings** — language toggle (see Localization below), app
   version, items that fire an implicit `Intent` (view/send) for: shop,
   contact, sponsors page on the live site — don't rebuild these as native
   screens for v1, link out.
...
- Use standard Android **string resources**: `res/values/strings.xml` (German,
  as the default) and `res/values-hr/strings.xml` (Croatian). Every
  user-visible string needs both a German and Croatian value — no English
  fallback strings left in shipped copy.
- Default language is **German** regardless of device locale (matches the
  ...
  locale). Use `AndroidX AppCompat`'s per-app language API
  (`AppCompatDelegate.setApplicationLocales`, backward-compatible via
  AppCompat 1.6+) so the override persists and applies immediately without
  restarting the app, and expose the toggle in Settings.

## Branding

- Colors (Croatian flag palette only — don't introduce other accent colors):
  `red = #CC0000`, `blue = #003580`, `white = #FFFFFF`. Define both a light
  and dark Compose `ColorScheme`.
- Headings font: **Clash Display** — variable TTF and static OTF weights are
  already available at `Fonts/ClashDisplay/` in the main website repo. Compose
  doesn't load `.otf` well in all toolchains — convert to `.ttf` if needed,
  place under `res/font/`, and define a Compose `FontFamily` for
...
- Body font: the website uses "Satoshi" but no Satoshi font files exist in the
  ...
  something. Use the Compose default (Roboto) for body text until Satoshi's
  actual font files are sourced and provided.
- App icon: base it on `logo.png` (club crest, transparent background) — build
  a proper adaptive icon (foreground/background layers per Android's adaptive
  icon spec). Treat generating the actual icon asset as a design task fed by
  that source image, not something to invent from scratch.
...
widget or WebView). No full offline parity requirement — caching the last
successfully fetched JSON (e.g. in a small Room table or DataStore blob) for
offline display is enough; don't build a sync engine.

## Before you start

Replace `{base}` throughout with the actual staging URL given above, and keep
it as a single configurable value (e.g. `BuildConfig` field per build variant)
so switching to production later is trivial.
